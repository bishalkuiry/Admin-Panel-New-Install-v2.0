...
(function () {
    ...
    function getConfig() {
        ...
        // Normalise every configured URL to a same-origin relative path so
        // that fetch() always talks to the host the browser is actually on,
        // regardless of the APP_URL value in .env (avoids CORS failures
        // when APP_URL is e.g. http://127.0.0.1:8000 but the real domain
        // is https://admin.example.com).
        function rel(u) {
            if (!u) return u;
            try { return new URL(u).pathname; } catch (e) { return u; }
        }
        var r = cfg.routes;
        for (var k in r) { if (r.hasOwnProperty(k)) r[k] = rel(r[k]); }
        var u = cfg.urls;
        for (var j in u) { if (u.hasOwnProperty(j)) u[j] = rel(u[j]); }
    ...
    window.appContentManager = function appContentManager() {
        ...
        return {
            ...
            init: function () {
                // Guard against double-init (Alpine 3 auto-calls init() if
                // it exists AND some views also specify x-init="init()").
            ...
            initLibrarySortable: function () {
                ...
                // Destroy previous instances to prevent duplicate event handlers
                // which block click events on the element cards.
                if (this._librarySortables && this._librarySortables.length) {
                    this._librarySortables.forEach(function (s) {
                        try { s.destroy(); } catch (e) { /* already destroyed */ }
                    });
                }
                this._librarySortables = [];
                var self = this;
                var containers = this.$el.querySelectorAll('.element-library-group');
                containers.forEach(function (container) {
                    self._librarySortables.push(Sortable.create(container, {
            ...
            loadPreviewItems: async function (tabId) {
                if (!tabId || !routes.previewItems) return;
                try {
            ...
            openAddModal: function (type, style, fromDrag) {
                ...
                this.formData = {
                    title: '', show_title: true, subtitle: '', show_subtitle: false,
                    is_active: true,
                    show_on_mobile: true, show_on_laptop: true,
                    style: style || 'style_1',
                    source: 'featured',
                    enable_background: false, background_type: 'color', background_color: '#ffffff', background_media_url: '',
            ...
            editContent: function (content) {
                ...
                this.formData = JSON.parse(JSON.stringify(Object.assign({}, content, {
                    show_on_mobile: content.show_on_mobile !== undefined ? Boolean(content.show_on_mobile) : true,
                    show_on_laptop: content.show_on_laptop !== undefined ? Boolean(content.show_on_laptop) : true,
            ...
            duplicateContent: async function (content) {
                ...
                var data = await res.json();
                if (data.success) {
                    var idx = this.contents.findIndex(function (c) { return c.id === content.id; });
                    if (idx !== -1) {
                        this.contents.splice(idx + 1, 0, data.data);
                    } else {
