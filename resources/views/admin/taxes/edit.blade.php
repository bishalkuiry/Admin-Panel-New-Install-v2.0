...
<form action="{{ route('admin.taxes.update', $tax) }}" method="POST" x-data="{ calcType: '{{ $tax->calculation_type }}' }" class="space-y-6">
    @csrf
    @method('PUT')
    ...
    <div class="flex items-center justify-between">
        <div class="flex items-center gap-4">
            ...
            <div>
                <h1 class="text-xl font-bold text-gray-900 tracking-wide">Edit Tax Rule</h1>
                <p class="text-sm text-gray-500 mt-1">Modify <span class="text-gray-900 font-semibold">{{ $tax->name }}</span></p>
            </div>
        </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div class="card bg-white border border-gray-200 shadow-sm rounded-xl p-6 h-fit">
            ...
            <div class="space-y-5">
                ...
                <div>
                    ...
                    <input type="text" name="name" value="{{ old('name', $tax->name) }}" class="w-full px-4 py-2 bg-white border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-primary-500/20 focus:border-primary-500 transition-all font-semibold" required>
                ...
                <div class="grid grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <div class="relative">
                            ...
                            <input type="number" name="value" value="{{ number_format(old('value', $tax->value), $priceDecimals, '.', '') }}" class="w-full px-8 py-2 bg-white border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-primary-500/20 focus:border-primary-500 transition-all font-semibold" step="{{ $priceStep }}" min="0" required>
                        </div>
                ...
                <div class="flex items-center justify-between p-4 border border-gray-200 rounded-xl bg-gray-50/50">
                    ...
                    <label class="relative inline-flex items-center cursor-pointer">
                        <input type="checkbox" name="is_inclusive" value="1" {{ $tax->is_inclusive ? 'checked' : '' }} class="sr-only peer">
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <div class="relative">
                            ...
                            <input type="number" name="min_order_value" value="{{ old('min_order_value', $tax->min_order_value) ? number_format(old('min_order_value', $tax->min_order_value), $priceDecimals, '.', '') : '' }}" class="w-full px-7 py-2 bg-white border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-primary-500/20 focus:border-primary-500 transition-all" step="{{ $priceStep }}" min="0" placeholder="0.00">
                        </div>
                    </div>
                    <div>
        ...
        <input type="hidden" name="is_active" value="{{ $tax->is_active ? 1 : 0 }}">
    </div>
</form>
@endsection
