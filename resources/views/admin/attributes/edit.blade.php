...
<div class="max-w-4xl">
    <div class="flex items-center gap-4 mb-6">
        ...
        <div>
            <h1 class="text-xl font-semibold text-gray-900">Edit Attribute</h1>
            <p class="text-sm text-gray-500">Update attribute details and values</p>
        </div>
    </div>
                    ...
                    <div class="card-body space-y-4">
                        <div class="grid grid-cols-2 gap-4">
                            <div>
                                ...
                                <input type="text" name="name" value="{{ old('name', $attribute->name) }}" class="input" required>
                            </div>
                            <div>
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            <div>
                                ...
                                <input type="text" value="{{ $attribute->slug }}" class="input bg-gray-50" disabled>
                            </div>
                            <div>
                                <label class="label">Sort Order</label>
                                <input type="number" name="sort_order" value="{{ old('sort_order', $attribute->sort_order) }}" class="input" min="0">
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        <div class="space-y-3">
                            <template x-for="(value, index) in values" :key="index">
                                <div class="flex items-center gap-3 p-4 bg-gray-50 rounded-lg border border-gray-100">
                                    ...
                                    <div class="flex-1">
                                        <input type="text" :name="'values[' + index + '][value]'" x-model="value.value" class="input" placeholder="Value name" required>
                                    </div>
                        ...
                        <div x-show="values.length === 0" class="text-center py-10">
                            ...
                            <button type="button" @click="addValue()" class="btn-secondary btn-sm mt-3">Add First Value</button>
                        </div>
                    </div>
                </div>
            </div>
                ...
                <div class="card">
                    ...
                    <div class="card-body space-y-3">
                        <label class="flex items-center gap-3 p-3 rounded-lg border border-gray-200 cursor-pointer hover:bg-gray-50 transition">
                            <input type="checkbox" name="is_filterable" value="1" {{ $attribute->is_filterable ? 'checked' : '' }} class="checkbox">
                            <div>
                        ...
                        <label class="flex items-center gap-3 p-3 rounded-lg border border-gray-200 cursor-pointer hover:bg-gray-50 transition">
                            <input type="checkbox" name="is_visible" value="1" {{ $attribute->is_visible ? 'checked' : '' }} class="checkbox">
                            <div>
                ...
                <div class="card">
                    ...
                    <div class="card-body space-y-3">
                        <div class="flex justify-between text-sm">
                            ...
                            <span class="font-medium text-gray-900">{{ $attribute->values_count ?? $attribute->values->count() }}</span>
                        </div>
                        <div class="flex justify-between text-sm">
                            <span class="text-gray-500">Created</span>
                            <span class="font-medium text-gray-900">{{ \App\Helpers\DateHelper::format($attribute->created_at) }}</span>
                        </div>
                        <div class="flex justify-between text-sm">
                            <span class="text-gray-500">Updated</span>
                            <span class="font-medium text-gray-900">{{ \App\Helpers\DateHelper::format($attribute->updated_at) }}</span>
                        </div>
                    </div>
                </div>
...
function attributeForm() {
    return {
        type: '{{ $attribute->type }}',
        values: @json($attribute->values->map(fn($v) => ['id' => $v->id, 'value' => $v->value, 'color_code' => $v->color_code ?? '#6366f1'])),
        addValue() {
            this.values.push({ id: null, value: '', color_code: '#6366f1' });
        },
        removeValue(index) {
